...
-- Internal evidence infrastructure for current solicitations/RFPs. This migration
-- deliberately does NOT make procurement records model-visible or score them as leads.
...
create index if not exists procurement_opportunities_active_deadline_idx
  on procurement.opportunities(active, response_deadline);
create index if not exists procurement_opportunities_state_idx
  on procurement.opportunities(place_state, active);
create index if not exists procurement_opportunities_solicitation_idx
  on procurement.opportunities(solicitation_number);
create index if not exists procurement_opportunities_source_idx
  on procurement.opportunities(source_slug, last_observed_at desc);
...
-- Register the first regional live RFP source and two follow-on source classes.
insert into ingest.sources (
  ...
  (
    'indiana-idoa-current-opportunities',
    'Indiana IDOA Current Business Opportunities',
    'Indiana Department of Administration',
    'procurement_opportunities',
    'Indiana state government',
    'official HTML current-opportunity table + linked bid documents',
    'daily',
    'state_authoritative',
    'active',
    'https://www.in.gov/idoa/procurement/current-business-opportunities/',
    'Current solicitations. Pair solicitation numbers with indiana-idoa-award-recommendations for post-award vendor/contract evidence.'
  ),
  (
    'louisville-renewable-contracts-archive',
    'Louisville Metro Renewable Contracts Archive',
    'Louisville Metro Government',
    'procurement_contracts',
    'Louisville Metro, Kentucky',
    'official archived renewable-contract report',
    'archived',
    'local_authoritative',
    'source_identified',
    'https://louisvilleky.gov/government/procurement',
    'Historical confirmed renewable-contract evidence; archived and not suitable as a live-current feed without a newer authoritative endpoint.'
  ),
  (
    'ohiobuys-public-solicitations',
    'OhioBuys Public Solicitations and Contracts',
    'State of Ohio',
    'procurement_opportunities',
    'Ohio state government',
    'official OhioBuys public portal; machine-readable endpoint pending',
    'continuous',
    'state_authoritative',
    'source_identified',
    'https://ohiobuys.ohio.gov/',
    'Includes public solicitations, awarded RFP/ITB records and master maintenance agreements. Do not automate around anti-bot/security controls; resolve an approved machine-readable source first.'
  )
...
-- Generic batched ingest. Loaders normalize date/string fields before calling this
-- function; source_payload retains the complete scoped source row/document metadata.
...
begin
  ...
  insert into ingest.raw_records (
    source_id, source_native_id, source_url, retrieved_at, observed_at,
    content_hash, parser_version, parse_status, provisional_entity_type,
    within_pilot_radius, raw_payload
  )
  select
    v_source_id,
    i.source_native_id,
  ...
  with incoming as (
    ...
    select distinct on (i.source_native_id)
      i.*,
      r.id as raw_id
    from incoming i
  ...
  insert into procurement.opportunities (
    source_record_id, source_slug, source_native_id, jurisdiction,
    solicitation_number, title, notice_type, base_type,
    buyer_name, buyer_subtier, buyer_office, buyer_code,
    naics_code, psc_code, set_aside,
    posted_at, response_deadline, archive_at, active,
    place_city, place_state, place_zip, place_country,
    description, canonical_url, contact, award, attributes,
  ...
  select
    ...
    nullif(r.normalized->>'notice_type',''),
    nullif(r.normalized->>'base_type',''),
    nullif(r.normalized->>'buyer_name',''),
    nullif(r.normalized->>'buyer_subtier',''),
    nullif(r.normalized->>'buyer_office',''),
    nullif(r.normalized->>'buyer_code',''),
    nullif(r.normalized->>'naics_code',''),
    nullif(r.normalized->>'psc_code',''),
    ...
    nullif(r.normalized->>'place_city',''),
    nullif(upper(r.normalized->>'place_state'),'') ,
    nullif(r.normalized->>'place_zip',''),
    nullif(r.normalized->>'place_country',''),
...
-- Deterministic classification only. These flags describe procurement scope and
-- vehicle shape; they are not opportunity scores and do not assert cleaning fit.
create or replace view procurement.v_facilities_contract_signals as
with base as (
  select
    o.*,
    lower(concat_ws(' ', o.title, o.description, o.notice_type, o.base_type,
                    o.buyer_name, o.buyer_subtier, o.buyer_office)) as corpus
  from procurement.opportunities o
), flags as (
  select
    b.*,
    (b.corpus ~ '(facilit(y|ies) (management|maintenance|support|operations)|base operations support|operations and maintenance|operations & maintenance|building maintenance|maintenance and repair|repair and maintenance|property management)') as is_facilities_management_scope,
    (b.corpus ~ '(on[- ]call|as[- ]needed|indefinite delivery|indefinite quantity|\bidiq\b|\bmatoc\b|\bsatoc\b|job order contract|job[- ]order contracting|\bjoc\b|task order|requirements contract|blanket purchase|\bbpa\b)') as is_on_call_vehicle,
    (b.corpus ~ '(statewide|multiple (sites|facilities|locations)|multi[- ]site|all (facilities|buildings|locations)|campus[- ]wide|base[- ]wide|installation[- ]wide|portfolio|district[- ]wide|system[- ]wide)') as is_multi_asset_scope,
    (b.corpus ~ '(paint(ing)?|repaint|coat(ing)?|pressure wash|power wash|exterior clean|facade|façade|masonry|tuckpoint|waterproof|sealant|caulk|roof|building envelope|concrete clean)') as is_surface_work_scope,
    (b.source_slug = 'sam-opportunities' and b.corpus ~ '(department of defense|department of the army|dept of the army|department of the navy|dept of the navy|department of the air force|dept of the air force|defense logistics agency|u\.s\. army|us army|u\.s\. navy|us navy|u\.s\. air force|us air force|marine corps|army corps of engineers|usace|space force)') as is_federal_military
  from base b
)
select
  f.*,
  (f.is_facilities_management_scope and (f.is_on_call_vehicle or f.is_multi_asset_scope)) as is_portfolio_unlock_candidate,
  case
...
select
  ...
  o.source_record_id,
  o.source_native_id as current_event_id,
  o.solicitation_number,
  o.title as opportunity_title,
  o.response_deadline,
  a.id as award_event_id,
  a.award_date,
  a.title as award_title,
  a.recommendation_status,
  a.estimated_contract_value,
  a.maximum_contract_value,
  s.vendor_name,
  s.vendor_organization_id,
  s.selected_value,
  s.maximum_value,
  s.selection_status,
  s.incumbent_signal,
  case when a.id is not null then 'solicitation_number_exact' else null end as match_method
from procurement.opportunities o
...
values
  ('sam-opportunities','pending_validation','Registered source has not yet passed first populated bulk-extract validation.',null,'sam-opportunities-v1','{}'::jsonb,now()),
  ('indiana-idoa-current-opportunities','pending_validation','Current-opportunity collector has not yet passed first live parse validation.',null,'indiana-idoa-current-v1','{}'::jsonb,now())
