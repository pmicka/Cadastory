-- Tighten physical-FM portfolio qualification.
-- Generic "all" is not enough to establish multi-site scope; it can refer to all units/equipment
-- within one or two named buildings. Require explicit various/multiple physical sites instead.
...
with base as (
  select
    ...
    (f.corpus ~ '((construction|renovation|repair|roofing|painting|coating).{0,80}(various|multiple).{0,50}(facilit(y|ies)|buildings|installations|sites|locations))|((various|multiple).{0,50}(facilit(y|ies)|buildings|installations|sites|locations).{0,80}(construction|renovation|repair|roofing|painting|coating))') as has_multi_site_physical_trade_phrase,
