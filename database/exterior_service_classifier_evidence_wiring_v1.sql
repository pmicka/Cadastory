-- Scout by Cadastory
-- Refresh wiring for exterior specialty classification.
-- Environmental proxies can raise biocide verification priority only;
-- verified facade evidence carries its own confidence into masonry support.
...
begin
  ...
  with facade as (
    select distinct on (m.building_source_record_id)
      ...
      src.slug facade_source_slug,
      o.facade_material,o.raw_facade_material,o.facade_material_status,o.observed_at
