...
The already-materialized 2026 window contains 18 HLSL30 and 64 HLSS30 scenes. Reading the published model bands plus Fmask once per granule would require approximately **912 band-window reads** for that current-year segment. Using the model's maximum 176 dates per sensor as a conservative full-previous-year bound adds at most 3,520 band-window reads, for an upper-bound planning figure of about **4,432** reads for the two-year target.
