...
- **FW-M45:** the authoritative event gate now requires event type + `Actual` NWS status + resolved alert footprint + active event time + a fresh successful Kentucky NWS poll. Production validation returned `not_applicable` with Kentucky HTTP 200, zero active Kentucky alerts, poll age 0 seconds, and zero unresolved qualifying geometries. A transactional stale-source QA forced the Kentucky poll age to 3,600 seconds and correctly returned `source_unavailable` instead of `inactive`. Ordinary severe thunderstorms/rain/wind cannot activate it.
...
- extreme-weather event context: `b3cc59d6cbc328214c5e311e6daea460764105e20b44644db833bef19487a41d`.
