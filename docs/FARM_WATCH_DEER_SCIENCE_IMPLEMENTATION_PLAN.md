...
Tier 1 production validation closed FW-M24 building density, FW-M36 road landscape context, FW-M39 exact 1/9 km² study scales, and FW-M45 the explicit extreme-event gate. FW-M45 was subsequently hardened so `not_applicable` requires a fresh successful jurisdiction NWS poll plus no qualifying intersecting event; stale/failed polling or unresolved qualifying alert geometry is `source_unavailable`. FW-M24 was subsequently hardened so each refresh records service edit currency, local feature production/imagery vintage ranges, attribute-coverage fractions, the source >450 sq ft inventory threshold, and explicit unquantified spatial completeness. FW-M36 was subsequently hardened with a production on-demand `road-focal-context-v1` evaluator that reproduces the study's 10 m distance-to-road transformation and 30/90/270 m focal means, so downstream step-selection work does not need a second road-measurement implementation. FW-M35 multiscale forest context is now implemented as a dedicated 10 m on-demand/product-center measurement at 30/90/270 m, so generic 500 m canopy rings no longer stand in for the Stephens forest covariate. Active work is therefore limited to the remaining autonomous or bounded one-time/static measurements such as woody canopy, snow/conifer context, Gallina concealment, stand vulnerability, managed food/water feature geometry, forest refuge classes, and potential-path agriculture.
