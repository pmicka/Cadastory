...
--
-- Keep the local parcel seeder restricted to providers backed by locally
-- ingested parcel records. Remote point-query providers are seeded separately.
...
declare
  ...
  v_profile_old text := 'on p.active and p.state_code=s.state_code and p.county_name=s.county_name';
  v_profile_new text := 'on p.active and p.state_code=s.state_code and p.county_name=s.county_name and p.provider_kind in (''pva_lrsn_html'',''embedded_parcel_owner'')';
  v_matches integer;
...
begin
  ...
  v_profile_matches := (length(v_def)-length(replace(v_def,v_profile_old,''))) / nullif(length(v_profile_old),0);
  if v_profile_matches <> 2 then
    raise exception 'expected exactly two responsible-party profile joins, found %',v_profile_matches;
  end if;

  v_def := replace(v_def,v_old,v_new);
  v_def := replace(v_def,v_profile_old,v_profile_new);
  if v_def not like '%eligible_source_kinds%'
     or v_def like '%q.source_kind in (''construction_window'',''exterior_cleaning'')%'
     or v_def not like '%p.provider_kind in (''pva_lrsn_html'',''embedded_parcel_owner'')%' then
    raise exception 'responsible-party provider/source-kind configuration patch did not apply cleanly';
  end if;
  execute v_def;
end
$$;
...
do $$
declare v_kinds jsonb; v_def text;
begin
  ...
  select pg_get_functiondef('public.internal_seed_responsible_party_resolution_jobs_v1(integer)'::regprocedure)
  into v_def;
  if v_def not like '%p.provider_kind in (''pva_lrsn_html'',''embedded_parcel_owner'')%' then
    raise exception 'local responsible-party provider boundary regression';
  end if;
end
$$;
