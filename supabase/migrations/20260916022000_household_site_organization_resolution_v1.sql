...
-- Resolve only a documented organization at the same non-residential site.
-- Person/household owners remain evidence-only and are never searched or contacted.
...
create table if not exists research.household_owner_fallback_deferrals(
  ...
  reason text not null check(reason in (
    'residential_household_no_business_target',
    'no_documented_site_organization',
    'ambiguous_site_organization'
  )),
...
create index if not exists household_owner_fallback_deferrals_active_idx
  on research.household_owner_fallback_deferrals(reason,requery_after)
...
declare
  v_jobs integer:=0;
  v_links integer:=0;
  v_residential_deferrals integer:=0;
begin
  ...
  create temporary table household_site_base on commit drop as
  ...
  select
    e.candidate_key,e.id owner_evidence_id,e.party_name owner_name,
    q.source_kind,q.priority,q.address_hint,q.missing_steps,q.state queue_state,
    s.state_code,s.county_name,s.primary_service_slug,s.source_id,s.details,
    case when q.source_kind='construction_window' then p.project_type else null end project_type,
    coalesce(
      nullif(s.details->>'city',''),
      nullif(rr.raw_payload->>'CITY','')
    ) city,
    coalesce(
      nullif(s.details->>'postal_code',''),
      nullif(s.details->>'zip',''),
      nullif(rr.raw_payload->>'ZIPCODE','')
    ) postal_code,
  ...
  left join intelligence.construction_service_windows w
   ...
   and split_part(e.candidate_key,':',2) ~ '^[0-9a-fA-F-]{36}$'
   and w.id=split_part(e.candidate_key,':',2)::uuid
  ...
  where e.rn=1
    ...
    and not exists(
      ...
      where x.candidate_key=e.candidate_key
  ...
  select candidate_key,'residential_household_no_business_target',source_kind,address_hint,
  ...
  from household_site_base
  where source_kind='construction_window'
    and project_type ilike 'Residential %'
  on conflict(candidate_key) do update set
    reason=excluded.reason,source_kind=excluded.source_kind,site_address_text=excluded.site_address_text,
    requery_after=excluded.requery_after,details=excluded.details,resolved_at=null,updated_at=now();
  get diagnostics v_residential_deferrals=row_count;

  create temporary table household_site_seed on commit drop as
  with eligible as (
    select b.*
    from household_site_base b
    where not (b.source_kind='construction_window' and b.project_type ilike 'Residential %')
      and not exists(
        select 1 from research.household_owner_fallback_deferrals d
        where d.candidate_key=b.candidate_key
    ...
    select
      state_code,address_key,
      'household-site:'||lower(coalesce(state_code,''))||':'||address_key cluster_key,
      max(address_hint) site_address_text,
      max(city) city,max(postal_code) postal_code,max(county_name) county_name,
      max(priority) queue_priority,
      ...
      md5(concat_ws('|',state_code,address_key,count(*)::text,max(priority)::text,
        string_agg(candidate_key,',' order by candidate_key))) input_fingerprint
    from eligible
    group by state_code,address_key
    order by max(priority) desc,count(*) desc,address_key
    limit p_limit
  )
  ...
  select
    'household_site_organization_v1','site_address',
    ...
    c.site_address_text,null,
    (-7000-(c.queue_priority*25)-least(cardinality(c.candidate_keys),20)*20)::integer,
    2,
    concat('"',c.site_address_text,'" ',coalesce(c.city,''),' ',coalesce(c.state_code,''),
      ' official organization business school church government location'),
    '[]'::jsonb,
    jsonb_build_object(
      'research_domain','household_site_organization',
      'site_address_text',c.site_address_text,
      ...
      'property_owner_is_person_or_household',true,
      'owner_identity_not_researched',true,
      ...
      'guardrails',jsonb_build_array(
        'exact_first_party_site_address_required',
        'person_or_household_owner_must_not_be_searched_or_contacted',
        'site_operator_is_not_automatically_the_buyer',
        'tenant_or_occupant_does_not_imply_exterior_maintenance_authority',
        'do_not_project_operator_identity_into_buyer_resolution',
        'do_not_infer_personal_email')
    ),now()
  from household_site_seed c
  on conflict(rule_pack,subject_type,subject_id) do update set
    ...
    display_name=excluded.display_name,priority=excluded.priority,search_query=excluded.search_query,
  ...
  insert into research.household_site_organization_job_candidates(
    job_id,candidate_key,owner_evidence_id,updated_at
  )
  select j.id,b.candidate_key,b.owner_evidence_id,now()
  from household_site_seed c
  join research.document_evidence_jobs j
    on j.rule_pack='household_site_organization_v1'
   and j.subject_type='site_address'
   and j.cluster_key=c.cluster_key
  join household_site_base b on b.candidate_key=any(c.candidate_keys)
  on conflict(job_id,candidate_key) do update set
    owner_evidence_id=excluded.owner_evidence_id,updated_at=now();
  get diagnostics v_links=row_count;

  return jsonb_build_object(
    'jobs_upserted',v_jobs,
    'candidate_links_upserted',v_links,
    'residential_deferrals_upserted',v_residential_deferrals,
    'eligible_address_clusters',(select count(*) from household_site_seed),
    'eligible_candidates',(select coalesce(sum(cardinality(candidate_keys)),0) from household_site_seed)
  );
end
$$;

revoke all on function public.internal_seed_household_site_organization_jobs_v1(integer)
  from public,anon,authenticated;
grant execute on function public.internal_seed_household_site_organization_jobs_v1(integer) to service_role;

create or replace function public.internal_claim_household_site_organization_jobs_v1(
...
begin
  ...
  where rule_pack='household_site_organization_v1'
    and state in ('queued','failed') and attempt_count>=max_attempts;
  ...
  with picked as (
    ...
    where rule_pack='household_site_organization_v1'
      ...
      and context->>'identity_projection'='suppressed'
      and coalesce((context->>'owner_identity_not_researched')::boolean,false)=true
      and p_search_available
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'id',c.id,'subject_key',c.subject_key,'display_name',c.display_name,
    'priority',c.priority,'attempt_count',c.attempt_count,'max_attempts',c.max_attempts,
    'search_query',c.search_query,'context',c.context
...
revoke all on function public.internal_claim_household_site_organization_jobs_v1(integer,boolean)
  from public,anon,authenticated;
...
create or replace function public.internal_complete_household_site_organization_job_v1(
  ...
  p_identity_confidence numeric default null,
...
begin
  ...
  if not found or v_job.rule_pack<>'household_site_organization_v1' or v_job.state<>'claimed' then
    raise exception 'claimed household site organization job required';
  end if;

  select coalesce(array_agg(candidate_key order by candidate_key),'{}'::text[])
  into v_candidates
  from research.household_site_organization_job_candidates
  where job_id=v_job.id;

  if p_outcome='completed' then
    if nullif(btrim(p_organization_name),'') is null then raise exception 'organization name required'; end if;
    if coalesce(p_identity_confidence,0)<.95 then raise exception 'identity confidence below threshold'; end if;
    if p_website_url !~ '^https?://' or p_source_url !~ '^https?://' then raise exception 'website/source URLs required'; end if;
    if coalesce(p_attributes->>'address_match_basis','')<>'exact_first_party_structured_address' then
      raise exception 'exact first-party structured address match required';
    end if;
    ...
    with matches as (
      ...
      where o.status='active' and public.scout_normalize_business_name(o.canonical_name)=v_norm
      union
    ...
    if v_match_count>1 then
      ...
      p_error:='multiple existing organizations match exact-address site identity';
    else
      if v_match_count=0 then
        insert into core.organizations(
          canonical_name,normalized_name,organization_type,status,website_url,attributes,updated_at
        ) values(
          btrim(p_organization_name),v_norm,null,'active',p_website_url,
          jsonb_build_object(
            'identity_source','household_site_exact_address_first_party',
            'identity_source_url',p_source_url,
            'identity_confidence',least(1,greatest(.95,p_identity_confidence)),
            'identity_scope','site_operator_candidate',
            'buyer_authority_not_implied',true,
            'property_owner_not_implied',true,
        ...
        set website_url=coalesce(website_url,p_website_url),
            attributes=attributes||jsonb_build_object(
              'last_site_identity_source_url',p_source_url,
              'last_site_identity_confidence',least(1,greatest(.95,p_identity_confidence)),
              'buyer_authority_not_implied',true
            ),updated_at=now()
        where id=v_org_id;
      end if;
      ...
      insert into scout.opportunity_responsible_party_evidence(
        candidate_key,party_role,party_name,party_name_normalized,party_kind,organization_id,
        site_address_text,evidence_class,confidence,source_url,source_authority,observed_on,
        attributes,updated_at
      )
      select l.candidate_key,'operator',btrim(p_organization_name),v_norm,'organization',v_org_id,
        v_job.context->>'site_address_text','documented',least(.99,greatest(.95,p_identity_confidence)),
        p_source_url,coalesce(nullif(p_source_authority,''),'First-party organization site'),current_date,
        coalesce(p_attributes,'{}'::jsonb)||jsonb_build_object(
          'resolution_scope','household_owner_fallback',
          'match_basis','exact_first_party_structured_address',
          'owner_identity_not_researched',true,
          'buyer_authority_not_implied',true,
          'ownership_not_implied',true,
          'property_manager_not_implied',true,
      ...
      on conflict(candidate_key,party_role,party_name_normalized,source_url) do update set
        ...
        source_authority=excluded.source_authority,observed_on=excluded.observed_on,
        attributes=excluded.attributes,updated_at=now();
      get diagnostics v_evidence=row_count;

      update research.household_owner_fallback_deferrals
      set resolved_at=now(),updated_at=now(),
          details=details||jsonb_build_object('resolved_by','exact_first_party_site_organization')
      where candidate_key=any(v_candidates) and resolved_at is null;
    end if;
  end if;

  if p_outcome='completed' then
    v_state:='completed';v_reason:=null;v_requery:=null;
  elsif p_outcome='needs_review' then
    v_state:='needs_review';v_reason:=coalesce(nullif(p_error,''),'ambiguous_site_organization');v_requery:=now()+interval '90 days';
  elsif p_outcome='no_evidence' and v_job.attempt_count>=v_job.max_attempts then
    v_state:='exhausted';v_reason:='no_documented_site_organization';v_requery:=now()+interval '90 days';
  elsif p_outcome='no_evidence' then
    v_state:='queued';v_reason:=null;v_requery:=null;
  elsif v_job.attempt_count>=v_job.max_attempts then
    v_state:='exhausted';v_reason:='retry_budget_exhausted';v_requery:=now()+interval '90 days';
  else
    v_state:='failed';v_reason:=null;v_requery:=null;
  end if;

  if v_state in ('needs_review','exhausted') then
    v_role:=case when v_state='needs_review' then 'ambiguous_site_organization' else 'no_documented_site_organization' end;
    insert into research.household_owner_fallback_deferrals(
      candidate_key,reason,source_kind,site_address_text,requery_after,details,resolved_at,updated_at
    )
    select l.candidate_key,v_role,q.source_kind,q.address_hint,v_requery,
      jsonb_build_object(
        'document_evidence_job_id',v_job.id,
        'job_state',v_state,
        'reason_detail',v_reason,
        'owner_identity_not_researched',true,
        'global_buyer_queue_not_blocked',true,
        'outbound_contact_performed',false
      ),null,now()
  ...
  update research.document_evidence_jobs
  set state=v_state,
      next_attempt_at=case when v_state='queued' then now()+interval '14 days' else next_attempt_at end,
      ...
      exhaustion_reason=v_reason,
      requery_after=v_requery,
      organization_name=case when v_org_id is not null then btrim(p_organization_name) else organization_name end,
      context=case when v_org_id is not null then
        jsonb_set(jsonb_set(context,'{organization_id}',to_jsonb(v_org_id::text),true),'{organization_name}',to_jsonb(btrim(p_organization_name)),true)
        else context end,
      outcome_metadata=coalesce(outcome_metadata,'{}'::jsonb)||jsonb_strip_nulls(jsonb_build_object(
        'organization_id',v_org_id,'operator_evidence_upserted',v_evidence,'identity_projection','suppressed',
        'buyer_authority_not_implied',true,'outbound_contact_performed',false,
        'evidence_excerpt',left(p_evidence_excerpt,1000)
      )),updated_at=now()
  where id=v_job.id;

  return jsonb_build_object(
    'job_id',v_job.id,'state',v_state,'organization_id',v_org_id,
    'candidate_count',cardinality(v_candidates),'operator_evidence_upserted',v_evidence,
    'identity_projection','suppressed','buyer_authority_not_implied',true,
    'outbound_contact_performed',false
  );
end
$$;

revoke all on function public.internal_complete_household_site_organization_job_v1(uuid,text,text,text,text,text,numeric,text,jsonb,text)
  from public,anon,authenticated;
grant execute on function public.internal_complete_household_site_organization_job_v1(uuid,text,text,text,text,text,numeric,text,jsonb,text)
  to service_role;
...
-- The household fallback is resolver-specific. It must not create buyer projection links.
do $$
