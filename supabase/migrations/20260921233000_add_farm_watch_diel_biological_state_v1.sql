...
create table if not exists farm_watch.deer_regional_breeding_evidence_v1 (
  ...
  exact_peak_start_md text check (exact_peak_start_md is null or exact_peak_start_md ~ '^(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])
...
(
  ...
  'Kentucky Fish and Wildlife peak breeding reference',
  'Kentucky Department of Fish and Wildlife Resources',
  'deer_breeding_phenology',
  'Kentucky',
  'Official Kentucky Fish and Wildlife public guidance stating that the mid-November modern gun season is designed to coincide with peak fall white-tailed deer breeding',
  'reference',
  'state_primary',
  'active_reference',
  'https://fw.ky.gov/News/Pages/Kentucky%E2%80%99s-Modern-Gun-Deer-Season-Opens-Nov.-13.aspx',
  'Public state wildlife-agency reference.',
  'public_government',
  'Supports statewide qualitative peak-breeding timing only. It does not establish an individual deer reproductive state or an exact conception date.'
),
(
...
values (
  ...
  null,
  'active',
  'Statewide qualitative context only: breeding occurs October through January and peak activity is usually in mid-November. The current contract does not infer an individual deer conception/estrus state and does not hard-code annual physiographic-region dates from secondary reproductions.'
)
...
as $$
  select jsonb_build_object(
    ...
    'relationship_ids',jsonb_build_array('FW-D05','FW-D06','FW-D21','FW-D22')
  );
$$;
...
begin
  ...
  if v_evidence_found and v_evidence.breeding_start_month is not null and v_evidence.breeding_end_month is not null then
  ...
  if v_state_code='KY' and v_evidence_found then
  ...
  return jsonb_build_object(
    ...
    'context',jsonb_build_object(
      ...
      'regional_reproductive_context',jsonb_build_object(
        ...
        'scope',case when v_evidence_found then v_evidence.evidence_id else 'unavailable' end,
        'peak_timing_label',case when v_evidence_found then v_evidence.peak_timing_label else null end,
        'evidence_class',case when v_evidence_found then v_evidence.evidence_class else null end,
        'source_ids',case when v_evidence_found then v_evidence.source_ids else '[]'::jsonb end,
        'ledger_ids',case when v_evidence_found then v_evidence.ledger_ids else '[]'::jsonb end,
      ...
      'applicable_relationship_ids',to_jsonb(
        (select array_agg(distinct u.id order by u.id) from unnest(v_ids) as u(id))
      ),
...
commit;
  ...
  exact_peak_end_md text check (exact_peak_end_md is null or exact_peak_end_md ~ '^(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])
  evidence_class text not null,
  source_ids jsonb not null default '[]'::jsonb,
  ledger_ids jsonb not null default '[]'::jsonb,
  source_url text,
