...
-- Farm Watch exact nested landscape-domain v2.
-- Preserve barrier-aware semantics while making 500 m ⊂ 1.5 km ⊂ 3 km exact by construction.
...
begin
  if p_product_kind='soils-map-units' then
    ...
    return jsonb_build_object(
      'algorithm_version','barrier-aware-landscape-domain-v2',
      'output_schema_version','farm-watch-landscape-domain-v1',
      'source_signature','zones_m=500,1500,3000|hard_barrier=matched_3dhp_named_flowline_to_intersecting_river_waterbody|fallback_flowline_buffer_m=30|component_rule=intersects_property|nesting=broad_first_exact_intersection'
    );
...
begin
  ...
  -- Derive broad first, then constrain each smaller domain to its parent.
  -- This preserves the exact barrier semantics while guaranteeing strict nesting.
