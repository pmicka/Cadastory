...
begin
  ...
  with wanted(product_kind,display_name,category,sort_order) as (
    values
      ...
      ('study-aligned-vegetation-height-context','Study-aligned vegetation height','physical_structure',15),
...
create or replace function public.farm_watch_get_managed_food_feature_context_v1_internal(
  p_slug text,
  p_as_of_date date default current_date
)
returns jsonb
...
as $$
  select farm_watch.farm_watch_get_managed_food_feature_context_v1_internal(
    p_slug,
    p_as_of_date
  );
$$;

revoke all on function public.farm_watch_get_managed_food_feature_context_v1_internal(text,date)
  from public,anon,authenticated;
grant execute on function public.farm_watch_get_managed_food_feature_context_v1_internal(text,date)
  to service_role;

create or replace function public.farm_watch_get_managed_water_source_context_v1_internal(
  p_slug text,
  p_as_of_date date default current_date
)
returns jsonb
...
set search_path='pg_catalog'
as $$
  select farm_watch.farm_watch_get_managed_water_source_context_v1_internal(
    p_slug,
    p_as_of_date
  );
$$;

revoke all on function public.farm_watch_get_managed_water_source_context_v1_internal(text,date)
  from public,anon,authenticated;
grant execute on function public.farm_watch_get_managed_water_source_context_v1_internal(text,date)
  to service_role;

comment on function farm_watch.farm_watch_get_deer_evidence_stack_v1_internal(text,date) is
'Returns the private neutral-evidence inventory for Farm Watch UI presentation, retaining production-validated study-aligned vegetation height alongside Tier 1 contexts and dated Surface Water State.';
