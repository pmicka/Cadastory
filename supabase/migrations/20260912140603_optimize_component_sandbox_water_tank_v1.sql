main:supabase/migrations/20260912140603_optimize_component_sandbox_water_tank_v1.sql
-- Keep the bounded v16 water-tank sandbox path on indexed, exemplar-scoped
-- lookups. The prior join to water.v_tank_geometry_profile ranked evidence for
-- every tank before the single exemplar was selected, which made cold calls
-- vulnerable to the API statement timeout and prompted ChatGPT retries/cards.
...
as $function$
  with target as (
    select
      ...
      ge.media_retained,
      'favorable'::text as operator_cleaning_geometry_assessment,
      'Trusted pilot-operator field expertise: single-pedestal geometry is comparatively favorable for drone cleaning.'::text as operator_assessment_basis,
      p.id as project_id,
      p.pnum,
      p.project_status,
    ...
    join lateral (
      ...
      where e.tank_id = t.id
        ...
        and (e.verify_after is null or e.verify_after >= current_date)
        and e.confidence >= 0.9
        and e.support_geometry = 'single_pedestal'
        and e.cross_bracing_status = 'none'
      order by
