...
begin
  if exists (
    select 1
    from cron.job
    where jobname='farm-watch-landscape-physical-pilot-v1'
  ) then
    perform cron.unschedule('farm-watch-landscape-physical-pilot-v1');
  end if;

  perform cron.schedule(
    'farm-watch-landscape-physical-pilot-v1',
    '17 * * * *',
    $cron$
      select farm_watch.farm_watch_refresh_landscape_physical_v1_internal(
