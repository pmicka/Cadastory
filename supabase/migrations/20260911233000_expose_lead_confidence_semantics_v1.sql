-- Expose separated confidence semantics through the existing additive opportunity spine contract.

create or replace function scout.get_opportunity_spine_projection_v2(p_candidate_key text)
returns jsonb
language sql
stable
set search_path = ''
as $function$
  select
    jsonb_set(
      coalesce(scout.get_opportunity_spine_projection(p_candidate_key), '{}'::jsonb),
      '{signal}',
      coalesce(scout.get_opportunity_spine_projection(p_candidate_key)->'signal','{}'::jsonb)
      || jsonb_strip_nulls(jsonb_build_object(
        'evidence_confidence', l.evidence_confidence,
        'confidence_semantics', 'evidence_reliability_not_lead_probability'
      )),
      true
    )
    || jsonb_build_object(
      'lead_assessment',
      jsonb_strip_nulls(jsonb_build_object(
        'lead_confidence', l.lead_confidence_label,
        'lead_confidence_reason', l.lead_confidence_reason,
        'action_window_open', l.action_window_open,
        'actionability_state', l.actionability_state,
        'actionability_reason', l.actionability_reason,
        'buyer_readiness', l.buyer_readiness,
        'buyer_readiness_reason', l.buyer_readiness_reason,
        'contract_version', l.semantics_contract_version,
        'calibration', 'categorical_not_probability'
      ))
    )
  from scout.v_opportunity_lead_semantics_v1 l
  where l.candidate_key = p_candidate_key;
$function$;

comment on function scout.get_opportunity_spine_projection_v2(text) is
  'Additive opportunity spine projection that explicitly separates evidence confidence, categorical lead confidence, actionability, and buyer readiness.';

revoke all on function scout.get_opportunity_spine_projection_v2(text) from public, anon, authenticated;
grant execute on function scout.get_opportunity_spine_projection_v2(text) to service_role;
