-- Replace the initial live-view address join with a small indexed temporary
-- facility cache. Portfolio evidence is bounded, but its source views are
-- relatively expensive; normalizing those rows once avoids a nested runtime
-- normalization scan during every responsible-party restore.

drop view if exists scout.v_documented_site_responsibility_candidates_v1;
