-- Batch 2: require causal service fit and a reachable buyer in addition
-- to a bounded clock for construction and farm prospecting. Dam condition,
-- inspection cadence, and FCC registration changes remain context until
-- corroborated by a purchasing or asset-specific service signal.
...
select
  ...
  case
    ...
    -- Construction status creates a prospecting window, not proof of unmet
    -- demand. Immediate action additionally requires a resolved reachable buyer.
    when s.source_kind = 'construction_window' then
      s.bounded_window_open and s.reachable_buyer

    -- Regional phenology can time outreach, but cannot make an unresolved
    -- field or farm actionable. Require a resolved reachable farm organization.
    when s.source_kind = 'farm_seasonal' then
      s.bounded_window_open and s.reachable_buyer

    -- Condition and projected inspection cadence are asset context. They do not
    -- establish an outsourced job or open buying window by themselves.
    when s.source_kind = 'dam_condition_cadence' then false

    -- An FCC registration event proves a registry change, not unmet inspection,
    -- mapping, or construction demand.
  ...
  case
    ...
    when s.source_kind = 'construction_window'
      and not s.reachable_buyer
      then 'buyer_route_needed'

    when s.source_kind = 'farm_seasonal'
      and not s.reachable_buyer
      then 'farm_buyer_or_operator_needed'

    when s.source_kind = 'dam_condition_cadence'
      then 'asset_condition_context_only'

    when s.source_kind = 'telecom_change'
      then 'registration_context_only'
    ...
    when s.source_kind in ('construction_window', 'farm_seasonal')
      and s.bounded_window_open and s.reachable_buyer
      then 'buyer_ready_prospecting_window'
  ...
  case
    ...
    when s.source_kind = 'farm_seasonal' and not s.reachable_buyer
      then 'Regional crop timing may guide market prospecting, but this farm lacks a resolved reachable operator or buyer.'
    ...
    when s.source_kind in ('construction_window', 'farm_seasonal')
      then 'A bounded prospecting period is open and Scout has a resolved reachable buyer; service need still requires qualification.'
...
comment on view scout.v_opportunity_actionability_v1 is
  'Canonical actionability overlay. Batch 2 requires causal service fit and a reachable buyer for construction/farm prospecting, and keeps dam condition/cadence plus FCC registration changes as context pending corroboration.';
