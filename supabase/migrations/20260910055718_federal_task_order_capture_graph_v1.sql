-- Scout by Cadastory
-- Federal task-order capture graph v1.
-- Additive/internal. Preserves the existing fail-closed contractor->asset posture.
...
select
  ...
  case
    ...
    when c.exact_asset_link_authorized and c.performance_window_status='upcoming' then 'upcoming_verified_task_scope'
    when c.exact_asset_link_authorized and c.performance_window_status='historical' then 'historical_verified_task_scope'
...
comment on view research.v_federal_task_order_capture_graph_v1 is
  'One row per substantive verified federal child order, modeled as buyer -> vehicle -> task order -> contractor -> asset context. Exact contractor/asset scope is authorized only after authoritative asset resolution; broad contractor-to-facility propagation remains forbidden.';
