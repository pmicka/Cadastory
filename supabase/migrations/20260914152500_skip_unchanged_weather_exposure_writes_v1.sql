-- Batch 6: keep weather exposure semantics unchanged while avoiding full rewrites
-- of durable rows when the computed exposure state has not changed.
...
begin
  ...
  drop table if exists pg_temp.weather_exposure_candidates;
  create temporary table weather_exposure_candidates on commit drop as
  select e.id event_id,
         wr.service_type_id,
         hr.indicator_id,
         rr.source_record_id,
         wr.asset_type,
         case e.evidence_class when 'post_event_footprint' then 'post_event_hazard_footprint_overlap' else 'nws_alert_polygon_overlap' end as exposure_basis,
         false as damage_confirmed,
         case e.evidence_class when 'post_event_footprint' then case when e.hazard_family='hail' then 0.78 else 0.82 end when 'validated_event' then 0.78 else 0.48 end as confidence,
         case e.evidence_class when 'post_event_footprint' then 'confirmed_hazard_exposure' else 'candidate' end as status,
         case e.evidence_class when 'post_event_footprint' then e.event_type||' footprint overlaps this asset; hazard exposure is supported, but asset damage is not proven.'
              else e.event_type||' alert polygon overlaps this asset; this is a provisional exposure candidate, not evidence of damage.' end as rationale,
         jsonb_build_object('hazard_family',e.hazard_family,'evidence_class',e.evidence_class) as attributes
  from intelligence.weather_events e
  ...
  select count(*) into e_candidate_count from pg_temp.weather_exposure_candidates;
  ...
  select c.event_id,c.service_type_id,c.indicator_id,c.source_record_id,c.asset_type,c.exposure_basis,c.damage_confirmed,c.confidence,c.status,c.rationale,c.attributes,now()
  from pg_temp.weather_exposure_candidates c
  left join intelligence.weather_asset_exposures x
    on x.event_id=c.event_id
   and x.service_type_id=c.service_type_id
   and x.source_record_id=c.source_record_id
  where x.id is null
     or (x.indicator_id,
         x.exposure_basis,
         x.confidence,
        ...
        (c.indicator_id,
         c.exposure_basis,
         greatest(x.confidence,c.confidence),
         c.status,
         c.rationale,
         c.attributes)
  on conflict(event_id,service_type_id,source_record_id) do update set
    ...
    confidence=greatest(intelligence.weather_asset_exposures.confidence,excluded.confidence),
    status=excluded.status,
    rationale=excluded.rationale,
    attributes=excluded.attributes,
    updated_at=now()
  where (intelligence.weather_asset_exposures.indicator_id,
         intelligence.weather_asset_exposures.exposure_basis,
         intelligence.weather_asset_exposures.confidence,
         intelligence.weather_asset_exposures.status,
         intelligence.weather_asset_exposures.rationale,
         intelligence.weather_asset_exposures.attributes)
        is distinct from
        (excluded.indicator_id,
         excluded.exposure_basis,
         greatest(intelligence.weather_asset_exposures.confidence,excluded.confidence),
         excluded.status,
         excluded.rationale,
         excluded.attributes);
  get diagnostics e_written=row_count;
  ...
  return jsonb_build_object(
    'service_windows_upserted',w_count,
    'asset_exposures_upserted',e_candidate_count,
    'asset_exposures_written',e_written,
    'asset_exposures_unchanged',greatest(e_candidate_count-e_written,0)
  );
end
$function$;