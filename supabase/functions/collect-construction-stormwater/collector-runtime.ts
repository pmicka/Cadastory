...
function serviceKey(): string {
  ...
  } catch { /* Legacy deployments remain supported. */ }
...
export function classifyResult(slug: string, body: Result, httpStatus: number): {
  outcome: Outcome; counts: Counts; errors: string[]
} {
  const errors: string[] = []
  ...
  const reruns = Array.isArray(body.rerun_preflights) ? body.rerun_preflights : []
  ...
  for (const x of reruns) if (x.error) errors.push(errorText(x.error))
  ...
  } else if (integer(body.counties_fetched) !== null) {
    succeeded = integer(body.counties_fetched)
    attempted = succeeded! + (Array.isArray(body.counties_failed) ? body.counties_failed.length : 0)
  } else {
    succeeded = integer(body.inserted) ?? integer(body.stored) ?? integer(body.recordsInserted)
    attempted = integer(body.processed_unit_pages) ?? integer(body.packages_attempted)
  }

  if (slug === 'faa-airspace-adapter' && body.snapshot) {
    const attrs = body.snapshot.attributes || {}
    const a = attrs.awareness_source_errors || {}
    const t = attrs.tfr_source_errors || {}
    const sourceErrors = [...Object.values(a), ...Object.values(t)]
    errors.push(...sourceErrors.map(errorText))
    attempted = 12
    failed = sourceErrors.length
    succeeded = Math.max(0, attempted - failed)
    if (body.snapshot.includes_awareness !== true || body.snapshot.includes_tfr !== true) {
      failed = Math.max(1, failed)
      succeeded = Math.min(succeeded, attempted - failed)
    }
  }

  let outcome: Outcome
  if (httpStatus < 200 || httpStatus >= 300 || body.ok === false || body.outcome === 'failed') outcome = 'failed'
  else if (failed > 0 || body.outcome === 'partial') outcome = succeeded === 0 ? 'failed' : 'partial'
  else if (attempted === 0 || body.outcome === 'no_work') outcome = 'no_work'
...
export function withCollectorRun(slug: string, handler: (req: Request) => Promise<Response>) {
  return async (req: Request): Promise<Response> => {
    ...
    const status = result.outcome === 'failed' ? (response.status >= 400 ? response.status : 502)
      : result.outcome === 'partial' ? 207 : response.status
    const watermark = body.latest_observed_at || body.latest_incident || body.data_as_of || body.observed_at || body.reportGenerated || null
    const summary = { outcome: result.outcome, counts: result.counts, errors: result.errors, source_watermark: watermark }
    const finished = await db.rpc('internal_finish_collector_run', {
      p_run_id: runId, p_status: result.outcome, p_http_status: status, p_summary: summary
    })
    ...
    return Response.json({ ...body, ok: result.outcome === 'succeeded' || result.outcome === 'no_work',
      outcome: result.outcome, run_id: runId, run_counts: result.counts }, { status })
  }
}
