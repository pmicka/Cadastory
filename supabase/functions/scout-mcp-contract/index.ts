...
const corsHeaders = {
  ...
  'cache-control':'no-store, max-age=0','pragma':'no-cache','referrer-policy':'no-referrer','x-content-type-options':'nosniff','x-scout-agent-contract':'v1',
}
...
function enrichTool(tool:any,contracts:Map<string,ContractRow>){const c=contracts.get(String(tool?.name||''));if(!c)return tool;const routing=typeof c.routing_description==='string'?c.routing_description.trim():'';const description=routing?[typeof tool?.description==='string'?tool.description.trim():'',routing].filter(Boolean).join('\n\n'):tool?.description;return {...tool,description,annotations:{...(tool.annotations||{}),...(c.annotations||{})},outputSchema:c.output_schema||tool.outputSchema,_meta:{...(tool._meta||{}),'scout/contractVersion':c.contract_version??1,'scout/responseType':c.response_type_slug??null,'scout/routingVersion':c.routing?.version??null,'scout/routingGroup':c.routing?.instruction_group??null,'scout/confirmation':c.routing?.confirmation??null}}}
...
Deno.serve(async(req:Request)=>{
  ...
  if(env?.method==='tools/list'){
    try{
      ...
      if(coreR.parsed){const merged=mergeToolEnvelopes(coreR.parsed,improveR.parsed,exploreR.parsed);const contracts=await manifest();return responseFromUpstream(req,coreR.upstream,enrichToolList(merged,contracts))}
