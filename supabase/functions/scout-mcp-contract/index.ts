...
const UI_LAB_URL = `${SUPABASE_URL}/functions/v1/scout-mcp-ui-lab`
...
function sandboxTool(){return {name:SANDBOX_TOOL,title:'Preview Scout Component Sandbox',description:'Owner-only read-only developer preview of the Scout MCP App component sandbox. Call only when the Scout owner explicitly asks to preview, surface, inspect, or test the sandbox UI. It uses placeholder content and no business data.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},_meta:{ui:{resourceUri:SANDBOX_RESOURCE_URI},'ui/resourceUri':SANDBOX_RESOURCE_URI,'openai/outputTemplate':SANDBOX_RESOURCE_URI,'openai/widgetAccessible':true}}}
...
Deno.serve(async(req:Request)=>{
  ...
  if(env?.method==='resources/read'&&String(env?.params?.uri||'')===SANDBOX_RESOURCE_URI)return await forwardOwnerSandbox(req,env,raw,inboundToken)
