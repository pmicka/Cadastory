...
const ADDRESS_PAGE_SIZE = 200
const MAX_ADDRESS_PAGES = 5
...
function h1Value(html: string) {
  // Jefferson PVA has a branded masthead <h1 ...> before the property address.
  // The property address is the plain <h1> inside the property-details content.
...
async function processJeffersonPva(job: Job) {
  ...
  if (!parsed.owner) return complete(job.id, 'no_evidence', { error: 'public property detail did not expose an owner' })
  ...
  if (expectedParcel && (!observedParcel || expectedParcel !== observedParcel)) {
    return complete(job.id, 'needs_review', { error: `parcel identity mismatch: expected ${expectedParcel}, observed ${observedParcel || 'missing'}` })
  }
...
async function fetchAddressPointPages(base: URL) {
  const features: O[] = []
  const urls: string[] = []
  let exhausted = false
  for (let page = 0; page < MAX_ADDRESS_PAGES; page++) {
    const queryUrl = new URL(base.toString())
    queryUrl.searchParams.set('resultOffset', String(page * ADDRESS_PAGE_SIZE))
    queryUrl.searchParams.set('resultRecordCount', String(ADDRESS_PAGE_SIZE))
    const payload = await fetchJson(queryUrl.toString())
    urls.push(queryUrl.toString())
    if (payload?.error) throw new Error(`LOJIC address-point query error: ${String(payload.error?.message || 'unknown').slice(0, 250)}`)
  ...
  return { features, urls, boundedComplete: exhausted }
}
...
async function processJeffersonPvaAddressRecovery(job: Job) {
  ...
  if (!houseMatch) return complete(job.id, 'needs_review', { error: 'site address does not begin with a numeric house number' })
  const houseNumber = Number(houseMatch[1])
  if (!Number.isSafeInteger(houseNumber) || houseNumber < 1) return complete(job.id, 'needs_review', { error: 'site address house number is invalid' })

  const baseQuery = new URL(addressLookupBase)
  baseQuery.searchParams.set('where', `${houseField}=${houseNumber}`)
  baseQuery.searchParams.set('outFields', `OBJECTID,${fullField},${parcelField},${lrsnField},${apartmentField}`)
  baseQuery.searchParams.set('returnGeometry', 'false')
  baseQuery.searchParams.set('orderByFields', 'OBJECTID ASC')
  baseQuery.searchParams.set('f', 'json')

  const pageResult = await fetchAddressPointPages(baseQuery)
  if (!pageResult.boundedComplete) {
    return complete(job.id, 'needs_review', { error: `LOJIC address-point query still exceeded ${MAX_ADDRESS_PAGES * ADDRESS_PAGE_SIZE} bounded records` })
  }
  ...
  for (const feature of pageResult.features) {
    ...
    exact.push({ fullAddress, matchedAddress: unitMatch && !baseMatch ? unitAddress : fullAddress, parcelId, lrsn, apartment })
  }
  if (!exact.length) return complete(job.id, 'no_evidence', { error: 'LOJIC address points returned no exact normalized site-address match' })
  ...
  if (!byParcel.size) return complete(job.id, 'needs_review', { error: 'exact LOJIC address match lacked parcel ID or LRSN' })
  if (byParcel.size !== 1) return complete(job.id, 'needs_review', { error: `exact LOJIC address matched ${byParcel.size} distinct parcels` })
  ...
  const html = await fetchHtml(sourceUrl)
  const parsed = parseJeffersonPva(html)
  if (!parsed.owner) return complete(job.id, 'no_evidence', { error: 'PVA detail for exact LOJIC address parcel did not expose an owner' })
  const observedParcel = normalizeParcel(parsed.parcelId)
  if (!observedParcel || observedParcel !== expectedParcel) {
    return complete(job.id, 'needs_review', { error: `LOJIC/PVA parcel identity mismatch: expected ${expectedParcel}, observed ${observedParcel || 'missing'}` })
  }
  return complete(job.id, 'completed', {
    party_name: parsed.owner,
    ...
    attributes: {
      ...
      address_point_query_url: pageResult.urls[0] || null,
      address_point_query_urls: pageResult.urls,
      address_point_pages_read: pageResult.urls.length,
...
async function processArcgisPointOwner(job: Job) {
  ...
  if (!base || !Number.isFinite(latitude) || !Number.isFinite(longitude) || !ownerField || !parcelField) return complete(job.id, 'needs_review', { error: 'ArcGIS owner lookup configuration incomplete' })
  if (latitude < -90 || latitude > 90 || longitude < -180 || longitude > 180) return complete(job.id, 'needs_review', { error: 'ArcGIS owner lookup coordinates invalid' })
  ...
  const payload = await fetchJson(queryUrl.toString())
  if (payload?.error) throw new Error(`ArcGIS parcel query error: ${String(payload.error?.message || 'unknown').slice(0, 250)}`)
  const features = Array.isArray(payload?.features) ? payload.features : []
  if (features.length === 0) return complete(job.id, 'no_evidence', { error: 'public parcel point query returned no parcel' })
  if (features.length !== 1) return complete(job.id, 'needs_review', { error: `public parcel point query returned ${features.length} parcels` })
  ...
  const parcelId = String(values?.[parcelField] ?? '').trim()
  if (!owner) return complete(job.id, 'no_evidence', { error: 'public parcel point query returned no owner' })
  if (!parcelId) return complete(job.id, 'needs_review', { error: 'public parcel point query returned no parcel identifier' })
  return complete(job.id, 'completed', {
    party_name: owner,
    ...
    source_authority: job.source_authority,
    observed_on: null,
    attributes: { provider_kind: job.provider_kind, lookup_key: job.lookup_key, parcel_match_method: 'authoritative_arcgis_point_intersection', lookup_latitude: latitude, lookup_longitude: longitude, query_sr: querySr, owner_field: ownerField, parcel_id_field: parcelField, public_detail_fields_only: true, bulk_mirror: false, source_media_retained: false, buyer_authority_not_implied: true, property_manager_not_implied: true, operator_not_implied: true },
  })
}

async function processJob(job: Job) {
  if (job.provider_kind === 'pva_lrsn_html' && job.profile_attributes?.resolution_mode === 'address_point_lrsn_recovery') return processJeffersonPvaAddressRecovery(job)
...
async function beginRun(limit: number, claimScope: string) {
  const result = await db.rpc('internal_begin_collector_run', { p_slug: SLUG, p_request_summary: { limit, claim_scope: claimScope } })
...
async function finishRun(runId: string, status: 'succeeded' | 'partial' | 'failed' | 'no_work', httpStatus: number, summary: O) {
  const result = await db.rpc('internal_finish_collector_run', { p_run_id: runId, p_status: status, p_http_status: httpStatus, p_summary: summary })
    ...
    const outcomes = await Promise.all(group.map(async job => {
      try {
        ...
        try {
          ...
          return { job_id: job.id, lookup_key: job.lookup_key, provider_kind: job.provider_kind, status: 'failed', error: `${message}; failure completion also failed: ${err(completionError)}` }
        }
      }
    }))
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const claimRpc = claimScope === 'address_recovery' ? 'internal_claim_responsible_party_address_recovery_jobs_v1' : 'internal_claim_responsible_party_resolution_jobs_v1'
    ...
    const summary = { outcome, counts: { attempted: jobs.length, succeeded, failed: failed.length }, providers: [...new Set(jobs.map(x => x.provider_kind))], claim_scope: claimScope, errors: failed.map(x => x.error).slice(0, 10) }
    ...
    const message = err(error)
    if (runId) try { await finishRun(runId, 'failed', 500, { outcome: 'failed', counts: { attempted: null, succeeded: null, failed: 1 }, claim_scope: claimScope, errors: [message] }) } catch {}
