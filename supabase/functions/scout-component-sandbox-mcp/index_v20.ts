...
const RESOURCE_URI = 'ui://scout/component-sandbox/v4'
...
function makeServer(){
  const server=new McpServer({name:'Scout Component Sandbox',version:'4.0.0'})
  registerAppTool(server,TOOL_NAME,{
    title:'Preview Scout Component Sandbox',
    description:'Owner-only read-only developer preview of the Scout MCP App component sandbox. Call only when the Scout owner explicitly asks to preview, surface, inspect, or test the sandbox UI. The View establishes one MCP Apps root connection during initialization. Slide 2 shows bounded property/portfolio geography. Slide 3 is a progressive lead contact card. Eligible contact names can be exported as standard .vcf files through the connected host-native download channel. Missing enrichment remains explicit rather than fabricated.',
    ...
    return {content:[{type:'text',text:'Scout component sandbox v1. The View uses one MCP Apps root connection for its host lifecycle and host-mediated actions. Eligible lead contacts use that connection for in-app .vcf downloads; no contact file is persisted by Scout.'}],structuredContent:{surface:'scout_component_sandbox',version:'v1',business_data:true,interaction_scope:'ephemeral_only'},_meta:{'openai/widgetSessionId':widgetSessionId,viewUUID:widgetSessionId}}
  })
  registerAppResource(server,'scout-component-sandbox',RESOURCE_URI,{mimeType:RESOURCE_MIME_TYPE},async()=>{
    ...
    return {contents:[{uri:RESOURCE_URI,mimeType:RESOURCE_MIME_TYPE,text:buildComponentSandboxHtml(targets),_meta:{ui:{prefersBorder:false,csp:{connectDomains:[],resourceDomains:[BASEMAP_ORIGIN,MCP_APP_SDK_ORIGIN]}},'openai/widgetDescription':'Owner-only Scout component sandbox using a single root MCP Apps View connection. Slide 2 distinguishes resolved assets from documented portfolio territory. Slide 3 is a normalized contact card. Eligible organization and named-person contacts request standard .vcf downloads through the already-connected MCP Apps host channel.','openai/widgetPrefersBorder':false,'openai/widgetCSP':{connect_domains:[],resource_domains:[BASEMAP_ORIGIN,MCP_APP_SDK_ORIGIN],redirect_domains:[DOWNLOAD_ORIGIN]}}}]}
  })
  return server
}
...
Deno.serve(async(req:Request)=>{if(req.method==='OPTIONS')return new Response(null,{status:204,headers:headers()});const download=await handleVcardDownloadRequest(req,SERVICE_KEY);if(download)return download;if(!(await authenticateOwner(req)))return Response.json({error:'not found'},{status:404,headers:headers()});try{const response=await mcpHandler.fetch(req);return new Response(response.body,{status:response.status,statusText:response.statusText,headers:headers(response.headers)})}catch(e){console.error('Scout component sandbox MCP error',e);return Response.json({error:'Scout component sandbox request failed'},{status:500,headers:headers()})}})
