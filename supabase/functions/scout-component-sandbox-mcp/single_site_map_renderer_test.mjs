...
assert.ok(server.includes('SANDBOX_MAP_CENTERS'))
assert.ok(server.includes('WARREN_PORTFOLIO_TILE_BOUNDS'))
assert.ok(server.includes('csp: { resourceDomains: [MAP_TILE_ORIGIN] }'))
assert.ok(server.includes('loadEmbeddedSandboxTiles'))
assert.ok(server.includes('MAX_EMBEDDED_RASTER_TILES = 32'))
...
assert.ok(server.includes("const RESOURCE_URI = 'ui://scout/component-sandbox/v28'"))
