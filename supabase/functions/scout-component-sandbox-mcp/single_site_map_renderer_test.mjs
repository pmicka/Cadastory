...
assert.ok(rendererSource.includes("document.createElement('canvas')"))
...
assert.ok(view.includes("scout-component-sandbox-mcp/map-tile/{z}/{x}/{y}.png"))
...
assert.ok(server.includes('const MAP_TILE_ORIGIN = SUPABASE_URL'))
assert.ok(server.includes("const MAP_TILE_UPSTREAM = 'https://a.tile.openstreetmap.fr/hot'"))
assert.ok(server.includes('parseSandboxTile'))
assert.ok(server.includes('isScoutSandboxRasterTileAllowed'))
assert.ok(server.includes('SCOUT_SANDBOX_PORTFOLIO_MANIFEST'))
assert.ok(server.includes('csp: { resourceDomains: [MAP_TILE_ORIGIN] }'))
assert.equal(server.includes('loadEmbeddedSandboxTiles'), false)
...
assert.ok(server.includes('const RESOURCE_URI = SCOUT_SANDBOX_RESOURCE_URI'))
assert.ok(server.includes('scoutSandboxCompatibilityResourceUris()'))
assert.equal(server.includes('scoutSandboxCompatibilityUsesEmbeddedRaster'), false)
...
assert.ok(mapContract.includes('a.tile.openstreetmap.fr/hot'))
