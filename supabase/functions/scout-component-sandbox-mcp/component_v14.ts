...
function replaceRequired(source:string, needle:string, replacement:string, label:string){
  if(!source.includes(needle))throw new Error('Scout component sandbox v14 transform failed: '+label)
...
const newDownload="async function downloadCurrentVcard(){const card=currentContactCard;if(!card||!vcardEligibility(card))return;const download=document.getElementById('contactVcardDownload'),copy=document.getElementById('contactVcardCopy'),live=document.getElementById('live'),host=window.openai,originalLabel=download?.textContent||'Download .vcf';let stage='bridge';if(download){download.disabled=true;download.textContent='Preparing…'}if(copy)copy.textContent='Preparing contact file…';try{if(!host?.callTool)throw new Error('host_tool_api_unavailable');const targetKey=String(mapTarget?.key||'').trim();if(!targetKey)throw new Error('target_key_unavailable');const routeKey=card?.resolution_status==='named_routing_contact'?String(card?.routes?.[0]?.key||'').trim()||null:null;stage='tool';const result=await host.callTool('scout_prepare_contact_vcard_download',{target_key:targetKey,route_key:routeKey});let data=result?.structuredContent||null;if(!data&&typeof result?.result==='string'){try{data=JSON.parse(result.result)}catch{}}if(!data&&Array.isArray(result?.content)){const textPart=result.content.find(part=>part?.type==='text'&&typeof part?.text==='string');if(textPart){try{data=JSON.parse(textPart.text)}catch{}}}stage='url';const href=String(data?.download_url||'').trim();if(!/^https:\\/\\//i.test(href))throw new Error('download_url_unavailable');stage='open';if(host.openExternal)await host.openExternal({href});else window.open(href,'_blank','noopener,noreferrer');hideVcardPrompt();if(live)live.textContent='Contact download opened.'}catch(error){console.error('Scout vCard download handoff failed at '+stage,error);if(copy){copy.textContent=stage==='bridge'?'ChatGPT did not expose the app tool bridge.':stage==='tool'?'ChatGPT blocked the contact helper call.':stage==='url'?'Scout did not return a usable contact download URL.':'ChatGPT blocked opening the contact download.'}if(live)live.textContent='Contact download failed at '+stage+'.'}finally{if(download){download.disabled=false;download.textContent=originalLabel}}}"
...
export function buildComponentSandboxHtml(targets: SandboxMapTarget[] = []) {
  ...
  html=replaceRequired(html,oldDownload,newDownload,'staged vCard host diagnostics')
  return html
}
