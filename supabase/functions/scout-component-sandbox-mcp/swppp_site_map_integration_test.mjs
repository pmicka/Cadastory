...
assert.ok(server.includes("text: staticScoutViewHtml("))
assert.ok(server.includes(".replace('__SCOUT_EMBEDDED_RASTER_TILES__', '[]')"))
assert.ok(server.includes("_meta: { 'scout/rasterTiles': await loadEmbeddedRasterTilesForSelection(selectedType, map) }"))
