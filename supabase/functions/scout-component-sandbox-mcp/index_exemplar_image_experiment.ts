...
} from './component_exemplar_image_experiment.ts'
...
const RESOURCE_URI = 'ui://scout/component-sandbox/exemplar-image-static-v1'
...
function makeServer(){
  const server=new McpServer({name:'Scout Component Sandbox',version:'3.6.1-image-exp'})
  registerAppTool(server,TOOL_NAME,{
    title:'Preview Scout Component Sandbox',
    description:'Owner-only read-only developer preview of Scout. This isolated image experiment uses the exact last-known-good sandbox renderer and replaces only carousel card 1 with one server-generated USGS/NAIP aerial image for the first single-property exemplar. No image analysis, storage, client-side image lifecycle code, or dynamic target switching is used.',
    ...
    return {content:[{type:'text',text:'Scout isolated exemplar imagery test. The base renderer is pinned to the last-known-good sandbox; only the first carousel card is replaced with a static USGS/NAIP image. No image bytes are persisted or analyzed.'}],structuredContent:{surface:'scout_component_sandbox',version:'v1',business_data:true,interaction_scope:'ephemeral_only'},_meta:{'openai/widgetSessionId':widgetSessionId,viewUUID:widgetSessionId}}
  })
  registerAppResource(server,'scout-component-sandbox',RESOURCE_URI,{mimeType:RESOURCE_MIME_TYPE},async()=>{
    ...
    return {contents:[{uri:RESOURCE_URI,mimeType:RESOURCE_MIME_TYPE,text:buildComponentSandboxHtml(targets),_meta:{ui:{prefersBorder:false,csp:{connectDomains:[],resourceDomains:[BASEMAP_ORIGIN,IMAGERY_ORIGIN,MCP_APP_SDK_ORIGIN]}},'openai/widgetDescription':'Isolated Scout exemplar-image test. The last-known-good sandbox renderer remains intact; card 1 is a server-generated static USGS/NAIP image for the first property exemplar. Cards 2 and 3 retain the baseline map/contact behavior. No image analysis or Scout image storage occurs.','openai/widgetPrefersBorder':false,'openai/widgetCSP':{connect_domains:[],resource_domains:[BASEMAP_ORIGIN,IMAGERY_ORIGIN,MCP_APP_SDK_ORIGIN],redirect_domains:[DOWNLOAD_ORIGIN]}}}]}
  })
  return server
}
