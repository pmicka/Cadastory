...
export const FARM_WATCH_DEER_MEASUREMENT_RESOLUTION_DECISIONS:
  ...
  decision(
    ...
    'P1',
    true,
    'Building/development density is a geometric landscape covariate that can be derived from authoritative/local building footprints at the study scale.',
    'Retain source, completeness and vintage; absence from an incomplete footprint source is not proof of no development.',
    ['Cross-check footprint coverage against imagery/parcel context in the validation domain'],
  ),
  decision(
    'FW-M25-discrete-stand-hunt',
    'reproduce',
  ...
  decision(
    ...
    'P1',
    true,
    'Road geometry and proximity/density are directly reproducible from authoritative transportation or validated OSM geometry at the source scale.',
    'Preserve road class and source completeness; no universal positive/negative deer sign is attached to the neutral product.',
    ['Compare mapped roads with current imagery/operator paths in the validation domain'],
  ),
  decision(
    ...
    'P2',
    true,
    'The fixed 1 km² and 9 km² source windows can be reproduced exactly. The source hunting-unit scale is study-administrative context rather than a transferable Kentucky geometry and should be split from the fixed-scale measurements.',
    'Split the contract: reproduce 1 km² and 9 km² metrics; retain hunting-unit findings only as broad-scale context unless a scientifically comparable management unit is defined.',
    ['Unit-test exact area windows and scale labeling'],
  ),
  decision(
    'FW-M40-d16-escape-cover-types',
    'remain_unavailable',
  ...
  decision(
    ...
    'P2',
    true,
    'An active hurricane/tropical-cyclone or comparably explicit extreme event is an authoritative dated state that can be represented from official event products.',
    'Do not generalize the Hurricane Irma relationship to routine storms, rain, wind or ordinary severe-weather alerts.',
    ['Require explicit event type, footprint/timing and source authority'],
  ),
  decision(
