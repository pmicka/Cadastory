...
function sandboxTool(){return {name:SANDBOX_TOOL,title:'Preview Scout opportunity card',description:'Owner-only read-only developer tool that renders one bounded Scout MCP Apps opportunity card and matching map. Select premium_exterior, water_tank, swppp_site, the Warren County water_utility_portfolio exemplar, or the Don Franklin Auto dealership_group_portfolio exemplar; omission preserves the PNC Tower compatibility default.',inputSchema:{type:'object',properties:{opportunity_type:{type:'string',enum:['premium_exterior','water_tank','swppp_site','water_utility_portfolio','dealership_group_portfolio']}},additionalProperties:false},outputSchema:sandboxResultSchema(),annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},_meta:{ui:{resourceUri:SANDBOX_RESOURCE_URI}}}}
