...
function sandboxTool(){return {name:SANDBOX_TOOL,title:'Preview Scout Component Sandbox',description:'Owner-only read-only developer preview of the Scout MCP App component sandbox. Call only when the Scout owner explicitly asks to preview, surface, inspect, or test the sandbox UI. It uses placeholder controls plus bounded real exemplar map-framing data and renders no opportunity overlays.',inputSchema:{type:'object',properties:{},additionalProperties:false},outputSchema:{type:'object',properties:{surface:{type:'string',enum:['scout_component_sandbox']},version:{type:'string',enum:['v1']},business_data:{type:'boolean',enum:[true]},interaction_scope:{type:'string',enum:['ephemeral_only']}},required:['surface','version','business_data','interaction_scope'],additionalProperties:false},annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},_meta:{ui:{resourceUri:SANDBOX_RESOURCE_URI},'ui/resourceUri':SANDBOX_RESOURCE_URI,'openai/outputTemplate':SANDBOX_RESOURCE_URI,'openai/widgetAccessible':true}}}
function sandboxResource(){return {uri:SANDBOX_RESOURCE_URI,name:'scout-component-sandbox',title:'Scout Component Sandbox',description:'Owner-only Scout MCP App component preview with bounded exemplar map framing.',mimeType:'text/html;profile=mcp-app'}}
